...
The Windows installer contains the self-contained `win-x64` application. The two macOS disk images contain self-contained Apple Silicon or Intel `.app` bundles and matching native AppKit wallpaper helpers. Neither package bundles wallpaper images; users download their chosen collection from R2 after confirmation.

## Local verification

```powershell
.\scripts\build-release.ps1 -Version 0.1.0-beta.4 -RequireInstaller
```
...
```powershell
.\scripts\start-github-release.ps1 -Version 0.1.0-beta.4
```
...
4. Let the macOS jobs build the ad-hoc signed Apple Silicon and Intel DMGs.
5. Install each DMG on a matching real Mac and verify Gatekeeper override, menu-bar restore, wallpaper changes on every display and content access.
